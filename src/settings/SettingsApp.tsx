...
function CreditsTab({ t, isDev, draft, set }: { t: (k: string) => string; isDev: boolean; draft: any; set: (k: string, v: any) => void }) {
  const items: [string, string, string?][] = [
    ['Rocky character', 'Based on "Project Hail Mary" by Andy Weir. Rocky is an Eridian from Tau Ceti.'],
    ['Voice synthesis', 'Pocket TTS for voice cloning. Voice reference audio by the project author.'],
    ['Speech recognition', 'Moonshine ONNX (fast) and faster-whisper (accurate) by Reverie Labs.'],
    ['Language model', 'Ollama local inference. Model by crafteriumt.', 'https://ollama.com/crafteriumt/Rockyv8'],
    ['Framework', 'Electron, React 19, Vite, Tailwind CSS, Motion.'],
    ['Fonts', 'CindieMono (D weight), Manrope variable — open-source.'],
    ['Developer', 'Krzysztof. Built with Claude Code.'],
  ];
                ...
                {url && (
                  <button
                    ...
                    style={{
                      ...
                      color: 'var(--accent)', fontSize: 12, padding: '0 0 0 6px',
